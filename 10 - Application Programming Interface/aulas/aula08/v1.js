...
const createUser = (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    // const id = users[users.length - 1].id + 1; Só funciona se o arquivo ja tiver objetos dentro
    data.id = id;
    users.push(data);
...
const validateData = async (data) => {
  ...
  if (await checkIfNameExists(user.name)){
    throw new console.error(`The user ${user.name} already exists.`);
  }
  return user;
};
...
const validateUserId = async (req, res, next) => {
  const find = await findUser(Number(req.params.userId));
  if (! user){
